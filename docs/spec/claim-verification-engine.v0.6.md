...
Supersedes [v0.5](claim-verification-engine.v0.5.md), [v0.4](claim-verification-engine.v0.4.md), [v0.3](claim-verification-engine.v0.3.md), [v0.2](claim-verification-engine.v0.2.md) and [v0.1](claim-verification-engine.v0.1.md). Companion documents: [Acceptance Criteria](acceptance-criteria.md), [Custodian Pack Interface](custodian-pack-interface.md).

---

## 0a. What changed in v0.6

§9.2 held every discrete count to an exact match, and left unstated which way a figure exactly halfway between two values rounds. Both surfaced once the engine read scale words: *"registered job-seekers were 29 thousand"* is a count stated to the nearest thousand, and against a published count of `28,700` it came back **Contradicted**, and the claim *False* — a confident, sourced, wrong answer about a statement the record supports. Separately, the rounding test rounded half to even, a convention the implementation had chosen for the claimant: against a published `4.5` on a series with precision `0.1`, "4" was Verified and "5" Contradicted.

| Area | v0.5 | v0.6 |
|---|---|---|
| Discrete counts | Exact match, always | Exact match **at the precision the claim states**. A count written in full is still exact; one stated with a scale word or an approximation word is Verified, tagged `rounded`, when the published count rounds to it |
| Exact halves | Unstated | Round to either neighbour, for every measure |
| Approximation words | Not read | Words the pack's lexicon declares ("about", "roughly") widen a figure's stated precision to its last non-zero digit |
| Floor on stated precision | — | None, by decision |

**Why the precision is read, not inferred.** It comes from how the claim is written: the last digit written, a declared scale word after it, a declared approximation word before it. Trailing zeros written in full are digits, so `29,000` is a claim of exactness, and a vote tally of `61,000` is not read as "about sixty-one thousand". A claimant who means "about" has the words to say so, and the engine reads them.

**Why there is no floor.** A figure stated very coarsely verifies easily, and that is what its claimant asserted. Any floor would be a number someone chose, which is what §9.2 exists to prevent; the `rounded` tag, and the published count cited beside it, show the reader the exact figure.

**Why bounds are not approximations.** "Nearly", "over" and "up to" say which side of a figure the truth lies on. That is a comparison, and it belongs to comparative derivation (§9.7.2), not to rounding.

The draft this adopts, with the alternatives rejected, is [proposals/9.2-rounded-counts.md](proposals/9.2-rounded-counts.md). Pack interface v1.9 adds the lexicon's approximation words.

---

## 0b. What changed in v0.5
...
|---|---|---|
| **A** | The claim figure rounds to the published figure at the claim's own stated precision, **or** `Δ ≤ max(u/2, 0.5% relative)`. **[v0.6]** A published figure exactly halfway between two values at that precision rounds to either | **Verified** |
...
| Discrete counts (seats, votes, licences, recorded incidents) | **[v0.6]** Exact match at the precision the claim states. Let `s` be the unit of the last digit written, times any scale word the pack's lexicon declares after it; an approximation word it declares before the figure widens `s` to the last non-zero digit. A claim equal to the published count is **Verified**. A claim with `s > 1` that the published count rounds to at `s` is **Verified**, tagged `rounded`. Anything else is **Contradicted**. There is no relative tolerance, and a count written in full (`s = 1`) must match exactly. Declared per measure via `discrete` in the pack. |

**Stated precision is read, not inferred. [v0.6]** A claim's stated precision comes from how it is written: its last written digit, a scale word the pack's lexicon declares after it ("29 thousand", "£5bn"), and an approximation word it declares before it ("about 29,000", "roughly £5bn"), which widens the precision to the last non-zero digit written. Trailing zeros written in full are digits: a count written in full is a claim of exactness, and the engine does not guess that the claimant meant less. There is no floor on stated precision; a figure stated coarsely verifies as coarsely as it was stated, and the `rounded` tag and the cited figure show the reader how far that was. Bounds ("nearly", "over", "up to") are not approximations and are not read here.
...
3. **Derivation coverage.** §9.7.2 fixes a closed list of **six** derivation operations **[v0.5]**. Whether those six cover the implication patterns that actually occur in public claims is an empirical question, answerable only against a corpus. The failure mode is silent: an implication with no matching operation is simply never surfaced, so the system under-reports rather than misreports. That is the correct direction to fail, but the gap should be measured rather than assumed small.
   ...
   **[v0.5]** This question is now narrower than it was, and the narrowing is instructive. The first real claim tested against the engine exercised a pattern the list did not cover — an inferential connective rather than a causal one — which is one data point suggesting the gap is not negligible. Five operations were not chosen carelessly; they were simply chosen without a corpus. The remaining question is no longer *whether* patterns are missing but *which*, and that still needs measurement rather than another round of introspection.
   ...
   A second data point, from a supplied test set ([validation](../validation-ten-claims.md)): active-voice causal verbs. *"Vaccines cause autism"* surfaces nothing, because every causal trigger is a connective with the effect first ("B due to A"), and in *"A causes B"* the cause comes first. Covering it is not a vocabulary addition, which would state the relationship backwards, but an operation that knows which flank is the cause.
