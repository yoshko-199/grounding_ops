...
**Normative contract, v1.9** — companion to [`claim-verification-engine.v0.4.md`](claim-verification-engine.v0.4.md) §9.4.

> **v1.9** adds a required `approximation_words` list to §3.6 `lexicons`, for spec v0.6's §9.2: words that, written directly before a figure (or its unit prefix), mark it as approximate — *"about 29,000"*, *"roughly £5bn"*. They join the quantity element and widen its stated precision to the last non-zero digit written, so *"about 29,000"* is judged as *"29 thousand"* is. Required and may be empty; no numeral; none may also be a unit prefix, since both are read before the numeral. Bounds (*"nearly"*, *"over"*, *"up to"*) are not approximations and must not be declared here. Packs at v1.8 do not load against v1.9.

> **v1.8** adds scale words. §3.6 `lexicons` gain a required `scale_words` table: a closed set of scales (`thousand`, `million`, `billion`, `trillion`) → the words for each in this language, read straight after a numeral and before any unit, so *"£5bn"* and *"5 billion pounds"* are one element each rather than a bare 5. What a scale *means* — its power of ten, on the short scale — is fixed in the engine, so a pack names scales but carries no numeral and cannot redefine one. §3.3 `measures` gain an optional `published_scale` (the same closed set plus `one`, the default) for a series published "in thousands" or "£ million". Both sides of a comparison are brought to the same scale by an exact power of ten, internally only: the output quotes the claim as written and cites the figure as published. A claim stated with a scale is judged at the precision it was stated in — "£5bn" is five to the nearest billion. No scale word may also be a unit phrase, since both are read after the numeral. Packs at v1.7 do not load against v1.8.

> **v1.7** adds a required `unit_prefixes` table to §3.6 `lexicons`, the counterpart of v1.6's `unit_phrases` for units written *before* the numeral: *"£5"*, *"US$ 12"*, *"USD 5"*. v1.6 read only what follows a number, so a pound figure was still a bare number, compared against whatever measure bound. A declared prefix directly before a numeral (whitespace allowed) joins the quantity element, and the figure is compared only if every unit the claim states, before and after it, is the measure's `unit` — *"£4.2 percent"* names two and is not compared. Same rules as `unit_phrases`: required, may be empty, no numeral in a phrase, no phrase under two units within the table. It is a separate table so that position is declared rather than guessed. Packs at v1.6 do not load against v1.7.

> **v1.6** adds a required `unit_phrases` table to §3.6 `lexicons`: unit id → the phrases that name that unit in this language, where a unit id is the string a measure declares as its `unit` (§3.3). The number pattern read no unit beyond an English `%`, `percent` and `points`, so *"212 degrees Fahrenheit"* was decomposed as a bare 212, compared against a series published in Celsius, and a true statement came back *contradicted*. A declared phrase directly after a numeral now joins the quantity element (spec §9.7.1: the unit is part of what was asserted), and an element whose stated unit differs from its measure's is *unverified*, never compared: converting it would put a figure no custodian published into the comparison (§3). Required, and may be declared empty — an empty table keeps the pre-v1.6 behaviour of comparing a bare number, but a pack has to say so rather than inherit it. Phrases may carry no numeral, and no phrase may name two units. Packs at v1.5 do not load against v1.6.
...
| `published_scale` | no | **v1.8.** The scale the custodian publishes the series in: `one` (default), `thousand`, `million`, `billion` or `trillion`. Declare it for a table "in thousands" or "£ million", so that a claim stated with a scale word is compared at the same scale. Used only in the comparison; the figure renders as published |
...
| `unit_phrases` | yes | **v1.6.** Unit id → the phrases naming that unit in this language, e.g. `degrees_celsius = ["degrees celsius", "°c"]`. Declare units your measures are *not* published in too, where claims use them: that is how a figure in the wrong unit is recognised instead of read as a bare number. A phrase directly after a numeral (whitespace allowed) joins the quantity element, longest phrase first, ending at a word boundary, case-insensitively. An element whose unit differs from its measure's `unit` is *unverified* and never compared or converted. No numeral in a phrase; no phrase under two units. An empty table is valid and reads no unit |
| `approximation_words` | yes | **v1.9.** Words that, directly before a figure or its unit prefix, mark it as approximate, e.g. `["about", "around", "roughly"]`. Matched at a word boundary, case-insensitively, and joined to the quantity element. They widen the figure's stated precision to its last non-zero digit (spec §9.2). No numeral, and none that is also a unit prefix. Never a bound such as "nearly" or "over". An empty list is valid and reads none |
| `scale_words` | yes | **v1.8.** Scale (`thousand`, `million`, `billion`, `trillion`) → the words for it in this language, e.g. `billion = ["billion", "bn"]`. Read straight after a numeral (whitespace allowed), longest first, ending at a word boundary, and before any unit phrase, and joined to the quantity element. No numeral in a word, no word under two scales, and no word that is also a unit phrase. An empty table is valid and reads no scale |
| `unit_prefixes` | yes | **v1.7.** Unit id → the phrases written *before* a numeral in this language, e.g. `pounds_sterling = ["£", "GBP"]`. Read ending directly before the numeral (whitespace allowed), longest first, starting at a word boundary, case-insensitively, and joined to the quantity element. The figure is compared only if every unit stated around it, here and in `unit_phrases`, is the measure's `unit`. Same constraints as `unit_phrases`. An empty table is valid and reads no prefix |
