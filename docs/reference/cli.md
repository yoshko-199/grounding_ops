...
| `ROBUSTNESS SWEEP` | The flip table: each admissible alternative, and whether the conclusion holds under it. A `last_break` row appears only when a declared series break falls inside the retrieved span, and is anchored at the first observation on or after it. When the sweep could not run, the section says why, and says the verdict is capped for it only when it was: "The verdict is capped accordingly" appears only beside a capped verdict, and any other verdict reached without a sweep reads "The verdict does not rest on the sweep". Either way, the sweep not running is never a pass |
